...
struct icbmsg_config {
	...
	atomic_t *tx_allocated_bm;		/* Blocks allocaed locally but not sent.
						 * Bits set here have to be set in tx_usage_bm first.
...
#endif
{
	...
	if (dev_data->is_initiator) {
		...
		for (i = 0; i < NUM_EPT; i++) {
			ept = &dev_data->ept[i];
			LOG_DBG("  EPT%d state %d", i, atomic_get(&ept->state));
...
#endif
		for (i = 0; i < NUM_EPT; i++) {
			LOG_DBG("  EPT%u waiting %u", i, dev_data->waiting_bound[i]);
...
#define GET_ICMSG_MIN_SIZE(i, local_blocks, remote_blocks)					\
	(ICMSG_BUFFER_OVERHEAD(i) + BYTES_PER_ICMSG_MESSAGE * (local_blocks + remote_blocks))
