...
#define BITPOOL_ATOMIC_OP(_ptr, _old, _new, _bitcnt)                                       \
	for(bool __status = false, __first_run = true; __first_run; )                      \
 ...
 * to search and take number of free continous blocks of memory.
 *
 ...
 *
 * After the calculation is done the @ref bitpool_cas function should be used to store
 * new computed atomic bitmap.
 * Only if the operation succeeded we can continue with with the computed values, in other
 ...
 *       It is ment to be used together with @ref bitpool_cas.
 */
 ...
 * Function together with @ref bitpool_read to atomically change the value.
 ...
 * @param[in] source         Bitpool to search for a block of given value
 * @param     val            The bit value to search for
 * @param     requested_size Minumum number of continous bits of given value
 * @param     bitcnt         Number of bits in the bitfield
 *
 * @return Index of the first bit in a block or negative error value
 */
