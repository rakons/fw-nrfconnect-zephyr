...
def write_address_irq_vector_table(fp, vt, nv):
    fp.write("uintptr_t __irq_vector_table __used _irq_vector_table[%d] = {\n" % nv)
    for i in range(nv):
        func = vt[i]
...
def write_irq_handlers(fp, vt):
    func_list = set()
    for func in vt:
        if not isinstance(func, int):
            func_list.add(func)
    func_list.remove('_isr_wrapper')

    for func_name in func_list:
        fp.write('extern void {}(void);\n'.format(func_name))

def write_isr_handlers(fp, swt):
    func_list = set()
    for param, func in swt:
        if not isinstance(func, int):
            func_list.add(func)
    func_list.remove(swt_spurious_handler)

    for func_name in func_list:
        fp.write('extern void {}(const void *param);\n'.format(func_name))
